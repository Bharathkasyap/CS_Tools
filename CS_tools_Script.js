...
function wireTabs(root=document){
  root.querySelectorAll('.tabs .tab').forEach(t=>t.addEventListener('click',()=>{
    ...
    const box=t.closest('.p-4')||document;
...
function ipToInt(ip){
  ...
  if(a.length!==4||a.some(x=>!Number.isInteger(x)||x<0||x>255)) throw new Error('Enter IPv4 like 192.168.1.0');
  return ((a[0]<<24)>>>0)+(a[1]<<16)+(a[2]<<8)+a[3];
}
function intToIp(n){
  return [(n>>>24)&255,(n>>>16)&255,(n>>>8)&255,n&255].join('.');
}
function maskFromPrefixInt(p){
  if(p<0||p>32) throw new Error('Prefix 0..32');
  return p===0?0:(((0xFFFFFFFF<<(32-p))>>>0));
}
function parseMaskInput(input){
  const trimmed = input.trim();
  if (/^\d+$/.test(trimmed)) {
    const p = Number(trimmed);
    if (!Number.isInteger(p) || p < 0 || p > 32) throw new Error('Prefix 0..32');
    return { prefix: p, mask: intToIp(maskFromPrefixInt(p)) };
  } else {
    const octets = trimmed.split('.').map(x=>Number(x));
    if (octets.length !== 4 || octets.some(x=>!Number.isInteger(x)||x<0||x>255)) throw new Error('Enter valid subnet mask like 255.255.255.0');
    const maskInt = ipToInt(trimmed);
    const binary = maskInt.toString(2).padStart(32, '0');
    const prefix = binary.split('').reduce((sum, bit) => sum + (bit === '1' ? 1 : 0), 0);
    if (!/1*0*$/.test(binary)) throw new Error('Subnet mask must have contiguous 1s');
    return { prefix, mask: trimmed };
  }
}
document.getElementById('subnet-calc')?.addEventListener('click',()=>{
  ...
  try {
    ...
    const { prefix, mask } = parseMaskInput(maskInput);
    const maskInt = maskFromPrefixInt(prefix);
    const network = (ip & maskInt) >>> 0;
    const broadcast = (network | (~maskInt >>> 0)) >>> 0;
    const firstHost = prefix === 32 ? network : ((network + 1) >>> 0);
    const lastHost = prefix >= 31 ? broadcast : ((broadcast - 1) >>> 0);
    const hostCount = prefix === 32 ? 1 : (prefix === 31 ? 2 : Math.max(0, (broadcast - network - 1)));
    const wildcard = intToIp(~maskInt >>> 0);
    out.value = `IP Address:    ${ipStr}
Subnet Mask:   ${mask}
CIDR Notation: ${ipStr}/${prefix}
Network:       ${intToIp(network)}
Broadcast:     ${intToIp(broadcast)}
First Host:    ${intToIp(firstHost)}
Last Host:     ${intToIp(lastHost)}
Total Hosts:   ${hostCount}
Usable Hosts:  ${prefix >= 31 ? hostCount : Math.max(0, hostCount - 2)}
...
function parseDotted(str){
  ...
  const octets=parts.map(p=>{ const n=Number(p); if(!Number.isInteger(n)||n<0||n>255) throw new Error('Each octet 0..255'); return n; });
  return octets;
}
function toDotted(arr){
  return arr.join('.');
}
function wildcardFromMask(oct){
  return oct.map(o=>255-o);
}
function maskFromPrefix(pfx){
  ...
  for(let i=0;i<4;i++){
    ...
    o.push(parseInt(b.padEnd(8,'0'),2));
  }
  return o;
}
document.getElementById('wm-fromMask')?.addEventListener('click',()=>{
  const inp=document.getElementById('wm-subnet');
  const out=document.getElementById('wm-out-fromMask');
  try {
    out.value=toDotted(wildcardFromMask(parseDotted(inp.value)));
    toast('Calculated');
  } catch(e) {
    out.value='Error: '+e.message;
  }
...
document.getElementById('wm-fromCidr')?.addEventListener('click',()=>{
  const c=document.getElementById('wm-cidr');
  const om=document.getElementById('wm-out-mask');
  const ow=document.getElementById('wm-out-wild');
  try {
    const m=maskFromPrefix(c.value);
    om.value=toDotted(m);
    ow.value=toDotted(wildcardFromMask(m));
    toast('Calculated');
  } catch(e) {
    om.value=ow.value='Error: '+e.message;
  }
});
...
document.getElementById('dec-to-hex')?.addEventListener('click',()=>{
  ...
  if(!Number.isFinite(n)||n<0){
    out.value='Error: enter a non-negative number';
    return;
  }
...
document.getElementById('hex-to-dec')?.addEventListener('click',()=>{
  ...
  v=v.replace(/^0x/i,'');
  if(!/^[0-9a-f]+$/i.test(v)){
    out.value='Error: hex only 0-9 A-F';
    return;
  }
...
document.getElementById('ts-from-epoch')?.addEventListener('click',()=>{
  ...
  if(!Number.isFinite(v)){
    out.value='Error: epoch seconds number required';
    return;
  }
...
document.getElementById('ts-to-epoch')?.addEventListener('click',()=>{
  ...
  if(!/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(s)){
    out.value='Error: use YYYY-MM-DD HH:mm:ss';
    return;
  }
...
document.getElementById('calc-ale')?.addEventListener('click',()=>{
  const av=Number(document.getElementById('av').value.trim());
  const ef=Number(document.getElementById('ef').value.trim());
  const aro=Number(document.getElementById('aro').value.trim());
  const out=document.getElementById('ale-out');
  if(!Number.isFinite(av)||!Number.isFinite(ef)||!Number.isFinite(aro)||ef<0||ef>1||aro<0){
    out.value='Error: AV number, EF 0..1, ARO >= 0';
    return;
  }
  const sle = av * ef;
  const ale = sle * aro;
...
document.getElementById('re-run')?.addEventListener('click',()=>{
  ...
  try {
    ...
    out.value=matches.map(m=>`Match: ${m[0]} (index ${m.index})`
